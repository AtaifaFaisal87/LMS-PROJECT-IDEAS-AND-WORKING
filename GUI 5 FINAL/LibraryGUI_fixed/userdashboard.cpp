...
static QFrame* makeSep() {
    ...
    f->setStyleSheet("color:#B8956A;"); return f;
}
    ...
    : QMainWindow(parent), currentUser(user)
{
    ...
    logoutBtn->setStyleSheet("background:rgba(92,61,30,0.3); color:#F5F0E8;"
...
void UserDashboard::buildMembershipTab(QTabWidget* t) {
    ...
    memStatusLbl->setStyleSheet("font-size:16px; font-weight:bold; color:#7A1C2E;");
    memExpiryLbl = new QLabel("Expires: –");
    memExpiryLbl->setStyleSheet("color:#8A7560;");
...
void UserDashboard::refreshAll() {
    ...
    memStatusLbl->setStyleSheet(hasMem
        ? "font-size:16px;font-weight:bold;color:#2D5E1E;"
        : "font-size:16px;font-weight:bold;color:#A01020;");
    if (hasMem)
...
void UserDashboard::loadDashboard() {
    ...
    for (auto& txn : db.transactions) {
        ...
        statusItem->setForeground(overdue ? QColor("#A01020") : QColor("#2D5E1E"));
...
void UserDashboard::loadBooks() {
    ...
    for (auto& b : db.books) {
        ...
        copItem->setForeground(b.copies > 0 ? QColor("#2D5E1E") : QColor("#A01020"));
...
void UserDashboard::loadTransactions() {
    ...
    for (auto& t : db.transactions) {
        ...
        retItem->setForeground(t.returned ? QColor("#2D5E1E") : QColor("#C67C00"));
...
void UserDashboard::onBorrow() {
    ...
    dlg.setStyleSheet("background-color:#EDE8DC;");
...
void UserDashboard::onReserveTable() {
    ...
    dlg.setStyleSheet("background-color:#EDE8DC; color:#2C1A0E;");
    auto* fl = new QFormLayout(&dlg);
