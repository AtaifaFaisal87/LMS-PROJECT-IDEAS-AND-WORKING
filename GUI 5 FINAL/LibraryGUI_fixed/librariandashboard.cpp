...
LibrarianDashboard::LibrarianDashboard(QWidget* parent) : QMainWindow(parent) {
    ...
    banner->setStyleSheet(
        ...
        "stop:0 #5C1020, stop:1 #7A1C2E); border-radius:8px; padding:10px;");
...
void LibrarianDashboard::buildOverviewTab(QTabWidget* t) {
    ...
    hdr->setObjectName("sectionLabel"); hdr->setStyleSheet("color:#7A1C2E;font-size:20px;font-weight:bold;font-family:Georgia,serif;");
...
void LibrarianDashboard::loadUsers() {
    ...
    for (auto& u : db.users) {
        ...
        si->setForeground(u.isDeleted ? QColor("#A01020") : QColor("#2D5E1E"));
...
void LibrarianDashboard::loadBooks() {
    ...
    for (auto& b : db.books) {
        ...
        ci->setForeground(b.copies > 0 ? QColor("#2D5E1E") : QColor("#A01020"));
...
void LibrarianDashboard::loadTransactions() {
    ...
    for (auto& t : db.transactions) {
        ...
        si->setForeground(t.returned ? QColor("#2D5E1E") : overdue ? QColor("#A01020") : QColor("#C67C00"));
...
void LibrarianDashboard::loadReservations() {
    ...
    for (auto& r : db.reservations) {
        ...
        ai->setForeground(r.active ? QColor("#2D5E1E") : QColor("#8A7560"));
