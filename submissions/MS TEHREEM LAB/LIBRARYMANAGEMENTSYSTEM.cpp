...
// Prevents statements like BORROW LIMIT REACHED or BOOK ADDED from firing mid-load.
class RuntimeLogger
{
private:
...
// ---- RESERVATION TIME VALIDATION (library open 5 AM - 12 AM, no past times, end after start) ----
...
// ---- BALANCE (minimum Rs. 0, must be numeric) ----
...
// STATIC MEMBER DEFINITIONS(auto-incrementing ID and constants)
...
public:
    ...
    // polymorphic destructor to ensure proper cleanup of derived classes
    virtual ~User()
    {
        delete membership;
    }
    ...
    // overload << operator for easy printing of user details (calls printTo implemented by subclasses)
    ...
    // ================= BORROW =================
    // FIX: Uses getEffectiveBorrowLimit() so membership boost applies to concurrent limit too
    // Student: 3 (no membership) / 5 (with membership)
    // Faculty: 5 (no membership) / 7 (with membership)
    // Guest:   1 (no membership) / 2 (with membership)
    ...
    // FIX: Uses getEffectiveBorrowLimit() which considers both user type AND membership status
    // Student: 3 (no membership) / 5 (with membership)
    // Faculty: 5 (no membership) / 7 (with membership)
    // Guest:   1 (no membership) / 2 (with membership)
    bool canBorrowToday()
    ...
    // ---- BALANCE MANAGEMENT ----
    // Deduct fine amount from balance. Returns true if successful, false if insufficient balance.
    bool deductBalance(double amount)
    {
        total_fines_paid += amount; // Track total fines paid
        if (balance >= amount)
        {
            balance -= amount;
            return true;
        }
        balance -= amount; // Allow negative balance (debt)
        return false;
    }

    // Add amount to balance (e.g., when user tops up)
    void addBalance(double amount)
    {
        balance += amount;
    }

    // Check if user can borrow based on balance/fine restrictions
    bool canBorrowBasedOnBalance()
    {
        return balance > -MAX_UNPAID_FINE_LIMIT;
    }

    // Calculate total unpaid fines (live + stored) for this user
...
// Initialize static member for maximum unpaid fine limit
const double User::MAX_UNPAID_FINE_LIMIT = 500.0;
// Initialize static member for auto-incrementing user IDs
int User::next_id = 1;
...
void saveUserToFile(User *u)
{
    ...
    UserRecord ur = {}; // zero out everything first to avoid garbage data in unused fields
    ...
    // downcast to access specific fields based on user type, and copy them into the struct
    if (u->get_usertype() == "STUDENT")
    {
        Student *s = dynamic_cast<Student *>(u); //object slicing 
...
//=========rewrite entire users file (after edits)================
...
{
private:
    vector<Book *> BK;//for future flexibility when we add child classes of Book (e.g. EBook, PrintedBook) 
                      // we can store them in the same catalog without changing the structure. We can also easily add features like sorting, filtering, etc. by using vector's built-in functions and algorithms.
...
//=================rewrite entire books file (after edits)=================
...
public:
    ...
    // CALCULATE OVERDUE MINUTES returns 0 if book wasn't late or hasn't been returned yet
    ...
    // MARK AS RETURNED (sets return time, updates return date string, and returns overdue minutes for fine calculation)
    int markreturned()
    {
        returned = true;
...
//=================rewrite entire transactions file (after edits)=================
...
public:
    ...
    */
    ...
    Fines(int u, int b, int mins, User *user)
        : user_id(u), book_id(b)
    {
        minuteslate = (mins < 0) ? 0 : mins; //if true then mins is negative, so set to 0, else set to mins (return-time - due-time)
...
//=================rewrite entire fines file (after edits)=================
...
public:
    ...
    // string comparison works here because times are in HH:MM 
    ...
    // Separate function to print inactive reservations without the "ACTIVE" status
...
// STATIC INITIALIZATION FOR AUTO-INCREMENTING RESERVATION IDs
...
// =================rewrite entire reservations file (after edits)=================
void rewriteReservationsFile(vector<Reservation> &reservations)
{
    ...
    {
        // Write ALL reservations — active, expired, and cancelled
        ReservationRecord rr = {};
...
// STATIC INITIALIZATION FOR AUTO-INCREMENTING TABLE IDs
int Table::next_id = 1;
...
// =================rewrite entire tables file (after edits)=================
...
public:
    // max tables constant for validation in librarian menu (can be changed as needed, but should be sufficient for most libraries)
    ...
    void registerUserInLibrary()
    {
        ...
        if (newUser)
        {
            ...
            // ---- CHECK UNIQUE USERNAME (already existing) ----
            bool duplicate = true;
            while (duplicate)
            {
                duplicate = false;
    ...
    bool borrowBook(User *u, string title, int duration_minutes)
    {
        ...
        // Check balance/fine restriction
        ...
        rewriteUsersFile(users); // persist updated daily borrow count
        ...
        for (int i = 0; i < (int)transactions.size(); i++)
        {
                ...
                !transactions[i].isReturned())
            {
                ...
                if (overdue_mins > 0)
                {
                    ...
                    // Auto-deduct fine from balance
                    double balanceBefore = u->getBalance(); // balance before fine deduction
    ...
    //--------------------views for transactions, fines, reservations, and users--------------------
    // ---------------- VIEW ----------------
    ...
    {
        autoExpireReservations(); // clean up before showing
    ...
    bool deleteUser(int uid, vector<Transaction> &txns, vector<Fines> &fins, vector<Reservation> &res)
    {
        ...
        // Count this user's transactions and fines
        ...
        // Show user details before confirming
        ...
        // Mark as deleted (soft delete — data stays in users.dat)
        u->setIsDeleted(true);
        ...
        // Persist the updated flag to file
        rewriteUsersFile(users);
    ...
    // Automatically expire reservations that have passed their end time
...
//=============================================
//     SEARCH BOOKS MENU (BY VARIOUS CRITERIA)
...
//=============================================================
//     BOOK ACTIONS MENU (BORROW, RETURN, READ, DOWNLOAD)
...
//=====================================================
//     MEMBERSHIP MENU (VIEW STATUS, PURCHASE, CANCEL)
...
//==============================================================================
//     USER MAIN MENU (VIEW DASHBOARD, SEARCH BOOKS, RESERVATIONS, MEMBERSHIP)
...
//==================================================================
//   MANAGE BOOKS (ADD, REMOVE) AND MANAGE TABLES (ADD, VIEW, REMOVE)
