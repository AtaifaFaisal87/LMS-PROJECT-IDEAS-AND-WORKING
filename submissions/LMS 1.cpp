...
public:
    ...
    // Returns true if user can still borrow today (limit: 2 per day)
    bool canBorrowToday()
    {
...
public:
    ...
    bool borrowBook(User *u, string title, int duration_minutes)
    {
        ...
        if (!u->canBorrowToday())
            throw UserException("DAILY BORROW LIMIT REACHED! YOU CAN ONLY BORROW 2 BOOKS PER DAY.");
