...
public:
        ...
        for (int i = 0; i < (int)transactions.size(); i++)
        {
                ...
                !transactions[i].isReturned())
            {
                ...
                if (overdue_mins > 0)
                {
                    ...
                    double balanceBefore = u->getBalance() + finalFine; // since getBalance() is after deduction, add it back to show original balance
                    bool deducted = u->deductBalance(finalFine);
                    if (deducted)
                    {
                        cout << "FINE AUTO-DEDUCTED FROM BALANCE!" << endl;
                        cout << "CURRENT BALANCE  : Rs." << balanceBefore << endl;
                        cout << "FINE DEDUCTED    : Rs." << finalFine << endl;
    ...
    bool deleteUser(int uid, vector<Transaction> &txns, vector<Fines> &fins, vector<Reservation> &res)
    {
        ...
        // Block deletion if user has unreturned books
...
void librarian::manageUsers(library *lib)
{
    ...
    do
    {
        ...
        cout << "TOTAL USERS: " << lib->getUserCount() << endl;
