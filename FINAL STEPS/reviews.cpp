...
// CHANGE 1: silent flag to suppress destructor messages during file loading
...
public:
    // DEFAULT CONSTRUCTOR
    Review()
        : reviewID(next_id++), bookID(0), userID(0),
          username("UNKNOWN"), reviewText(""), rating(0), reviewTime(0) {}

    // PARAMETERIZED CONSTRUCTOR
    Review(int bid, int uid, string uname, string text, int r)
        : reviewID(next_id++), bookID(bid), userID(uid),
          username(uname), reviewText(text), rating(r)
    {
        reviewTime = getCurrentTime();
    }

    // RESTORE CONSTRUCTOR (for loading from file)
    Review(int id, int bid, int uid, string uname, string text, int r, time_t t)
        : reviewID(id), bookID(bid), userID(uid),
          username(uname), reviewText(text), rating(r), reviewTime(t) {}
    ...
    time_t getReviewTime() const { return reviewTime; }
    ...
    void setUsername(string u) { username = u; }
    void setReviewText(string t) { reviewText = t; }
    void setRating(int r) { rating = r; }
    void setReviewTime(time_t t) { reviewTime = t; }
    ...
    {
        os << "  ----------------------------------------" << endl
           << "  | REVIEW ID  : " << r.reviewID << endl
           << "  | BY         : " << r.username
           << " (USER ID: " << r.userID << ")" << endl
           << "  | RATING     : ";
        for (int i = 0; i < r.rating; i++) os << "*";
        for (int i = r.rating; i < 5; i++) os << "-";
        os << " (" << r.rating << "/5)" << endl
           << "  | REVIEW     : " << r.reviewText << endl
           << "  | DATE       : " << timeToString(r.reviewTime)
           << "  ----------------------------------------" << endl;
        return os;
    }
};
int Review::next_id = 1;

// ---- REVIEW FILE I/O ----
void saveReviewToFile(Review &rev)
{
    ofstream file("reviews.dat", ios::binary | ios::app);
    if (!file)
        throw FileException("ERROR: COULD NOT OPEN reviews.dat FOR WRITING!");

    ReviewRecord rr = {};
    rr.reviewID = rev.getReviewID();
    rr.bookID = rev.getBookID();
    rr.userID = rev.getUserID();
    rr.rating = rev.getRating();
    rr.reviewTime = rev.getReviewTime();
    strncpy(rr.username, rev.getUsername().c_str(), sizeof(rr.username) - 1);
    rr.username[sizeof(rr.username) - 1] = '\0';
    strncpy(rr.reviewText, rev.getReviewText().c_str(), sizeof(rr.reviewText) - 1);
    rr.reviewText[sizeof(rr.reviewText) - 1] = '\0';

    file.write((char *)&rr, sizeof(rr));
    file.close();
}

void loadReviewsFromFile(vector<Review> &reviews)
{
    ifstream file("reviews.dat", ios::binary);
    if (!file)
        return;
    ...
    {
        Review rev(rr.reviewID, rr.bookID, rr.userID,
                   rr.username, rr.reviewText, rr.rating, rr.reviewTime);
        reviews.push_back(rev);
        if (rr.reviewID >= maxID)
            maxID = rr.reviewID;
    }

    if (maxID >= Review::getNextID())
        Review::setNextID(maxID + 1);

    file.close();
}

void rewriteReviewsFile(vector<Review> &reviews)
{
    ofstream file("reviews.dat", ios::binary);
    if (!file)
        throw FileException("ERROR: COULD NOT OPEN reviews.dat FOR REWRITING!");

    for (int i = 0; i < (int)reviews.size(); i++)
    {
        ReviewRecord rr = {};
        rr.reviewID = reviews[i].getReviewID();
        rr.bookID = reviews[i].getBookID();
        rr.userID = reviews[i].getUserID();
        rr.rating = reviews[i].getRating();
        rr.reviewTime = reviews[i].getReviewTime();
        strncpy(rr.username, reviews[i].getUsername().c_str(), sizeof(rr.username) - 1);
        rr.username[sizeof(rr.username) - 1] = '\0';
        strncpy(rr.reviewText, reviews[i].getReviewText().c_str(), sizeof(rr.reviewText) - 1);
        rr.reviewText[sizeof(rr.reviewText) - 1] = '\0';
...
public:
    ...
    // DESTRUCTOR
    ~Book() { if (!silentMode) cout << "BOOK DESTROYED!" << endl; } // CHANGE 2: suppressed during loading
...
public:
    ...
    {
        BK.push_back(b);
         if (!silentMode) cout << "BOOK ADDED SUCCESSFULLY!" << endl;
    }
...
public:
    ...
    // DESTRUCTOR
    ~Transaction() { if (!silentMode) cout << "TRANSACTION DESTROYED!" << endl; } // CHANGE 3: suppressed during loading
...
*/
    ...
    // DESTRUCTOR
    ~Fines() { if (!silentMode) cout << "FINES DESTROYED!" << endl; } // CHANGE 4: suppressed during loading
...
private:
    ...
    vector<Review> reviews;           // book reviews (members only)
...
public:
    ...
    vector<Review> &getReviewsVector() { return reviews; }
    ...
    // ---------------- REVIEW INTERACTIONS ----------------
    bool addReview(User *u, int bookID, string text, int rating)
    {
        if (!u->hasMembership())
            throw UserException("YOU MUST HAVE AN ACTIVE MEMBERSHIP TO LEAVE A REVIEW!");

        Book *b = catalog.SearchbyID(bookID);
        if (!b)
            throw BookException("BOOK NOT FOUND WITH ID: " + to_string(bookID));

        if (rating < 1 || rating > 5)
            throw UserException("RATING MUST BE BETWEEN 1 AND 5!");

        reviews.emplace_back(bookID, u->get_userid(), u->getUsername(), text, rating);
        saveReviewToFile(reviews.back());
        cout << "REVIEW SUBMITTED SUCCESSFULLY!" << endl;
        return true;
    }

    void viewBookReviews(int bookID)
    {
        bool found = false;
        for (int i = 0; i < (int)reviews.size(); i++)
        {
            if (reviews[i].getBookID() == bookID)
            {
                if (!found)
                {
        ...
        if (!found)
            cout << "NO REVIEWS YET FOR THIS BOOK." << endl;
    }
    ...
    void viewAllReviews()
    {
        ...
        cout << "================ ALL BOOK REVIEWS ================" << endl;
        for (int i = 0; i < (int)reviews.size(); i++)
        {
            Book *b = catalog.SearchbyID(reviews[i].getBookID());
            cout << "  BOOK: " << (b ? b->get_title() : "UNKNOWN") << " (ID: " << reviews[i].getBookID() << ")" << endl;
            cout << reviews[i];
        }
    }
};
...
void User::searchBooks(library *lib)
{
    ...
    do
    {
        ...
        if (b != nullptr)
        {
            ...
            cout << *b;
            cout << "\n";
            lib->viewBookReviews(b->get_bookid());
        }
...
void User::bookActionMenu(library *lib)
{
    ...
    do
    {
        ...
        cout << "5. WRITE A REVIEW (MEMBERS ONLY)" << endl;
        cout << "6. EXIT" << endl;
        ...
        case 5:
        {
            // WRITE A REVIEW - members only
            int bid = getValidInt("ENTER BOOK ID TO REVIEW: ");
            Book *rb = lib->getCatalog()->SearchbyID(bid);
            if (!rb)
            {
                cout << "BOOK NOT FOUND!" << endl;
                break;
            ...
            cout << *rb;
            lib->viewBookReviews(bid);
            int rating = getValidIntInRange("ENTER YOUR RATING (1-5): ", 1, 5);
            string reviewText = getValidString("ENTER YOUR REVIEW: ");
            try
            {
                lib->addReview(this, bid, reviewText, rating);
            }
            catch (UserException &e)
            {
                cout << e.what() << endl;
...
void librarian::viewFullDetails(library *lib)
{
    ...
    do
    {
        ...
        case 6:
        {
            cout << "\n------------- BOOK CATALOG (WITH REVIEWS) -------------" << endl;
            BookCatalog *cat = lib->getCatalog();
            if (cat->get_bookcount() == 0)
            {
                cout << "NO BOOKS IN CATALOG!" << endl;
            }
            else
            {
                for (int bi = 0; bi < cat->get_bookcount(); bi++)
                {
                    Book *bk = cat->getBookAtIndex(bi);
                    if (bk)
                    {
                        cout << *bk;
                        lib->viewBookReviews(bk->get_bookid());
                        cout << endl;
                    }
                }
            }
...
{
    library lib;
    // CHANGE 5: enable silent mode before loading so destructor messages are suppressed at startup
    silentMode = true;
    try
    {
    ...
    try
    {
        loadReviewsFromFile(lib.getReviewsVector());
    }
    catch (FileException &e)
    {
        cout << "REVIEWS FILE ERROR: " << e.what() << endl;
    }
    silentMode = false; // restore normal output after loading
