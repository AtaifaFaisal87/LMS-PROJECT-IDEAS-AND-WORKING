    ...
    : QMainWindow(parent), currentUser(user)
{
    ...
    logoutBtn->setStyleSheet("background:rgba(255,255,255,0.2); color:#FFFFFF;"
...
void UserDashboard::buildMembershipTab(QTabWidget* t) {
    ...
    memStatusLbl->setStyleSheet("font-size:16px; font-weight:bold; color:#22D3EE;");
    memExpiryLbl = new QLabel("Expires: –");
    memExpiryLbl->setStyleSheet("color:#A5B4FC;");
...
void UserDashboard::refreshAll() {
    ...
    memStatusLbl->setStyleSheet(hasMem
        ...
        : "font-size:16px;font-weight:bold;color:#F43F5E;");
    if (hasMem)
...
void UserDashboard::loadDashboard() {
    ...
    for (auto& txn : db.transactions) {
        ...
        statusItem->setForeground(overdue ? QColor("#F43F5E") : QColor("#10B981"));
...
void UserDashboard::loadBooks() {
    ...
    for (auto& b : db.books) {
        ...
        copItem->setForeground(b.copies > 0 ? QColor("#10B981") : QColor("#F43F5E"));
