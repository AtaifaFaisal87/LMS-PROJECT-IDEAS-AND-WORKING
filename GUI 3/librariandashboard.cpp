...
static QFrame* libSep() {
    ...
    f->setStyleSheet("color:#3D5AF1;"); return f;
}
...
LibrarianDashboard::LibrarianDashboard(QWidget* parent) : QMainWindow(parent) {
    ...
    banner->setStyleSheet(
        ...
        "stop:0 #1E1B4B, stop:1 #3D5AF1); border-radius:8px; padding:10px;");
...
void LibrarianDashboard::buildOverviewTab(QTabWidget* t) {
    ...
    hdr->setObjectName("sectionLabel"); hdr->setStyleSheet("color:#22D3EE;font-size:20px;font-weight:bold;");
...
void LibrarianDashboard::loadOverview() {
    ...
    totalFinesLbl->setText(QString("Rs.%1").arg(totalFine,'f',0));
}
...
void LibrarianDashboard::loadUsers() {
    ...
    for (auto& u : db.users) {
        ...
        si->setForeground(u.isDeleted ? QColor("#F43F5E") : QColor("#10B981"));
        ...
        usersTable->setItem(row,8, new QTableWidgetItem(
            QString("Rs.%1").arg(u.balance,'f',0)));
    }
}
...
void LibrarianDashboard::loadBooks() {
    ...
    for (auto& b : db.books) {
        ...
        ci->setForeground(b.copies > 0 ? QColor("#10B981") : QColor("#F43F5E"));
...
void LibrarianDashboard::loadTransactions() {
    ...
    for (auto& t : db.transactions) {
        ...
        si->setForeground(t.returned ? QColor("#10B981") : overdue ? QColor("#F43F5E") : QColor("#F59E0B"));
...
void LibrarianDashboard::loadFines() {
    ...
    for (auto& f : db.fines) {
        ...
        finesTable->setItem(row,4, new QTableWidgetItem(QString("Rs.%1").arg(f.originalAmount,'f',2)));
        finesTable->setItem(row,5, new QTableWidgetItem(f.discountApplied ? "15%" : "–"));
        finesTable->setItem(row,6, new QTableWidgetItem(QString("Rs.%1").arg(f.finalAmount,'f',2)));
    }
}
...
void LibrarianDashboard::loadReservations() {
    ...
    for (auto& r : db.reservations) {
        ...
        ai->setForeground(r.active ? QColor("#10B981") : QColor("#9CA3AF"));
...
void LibrarianDashboard::onAddBook() {
    ...
    dlg.setStyleSheet("background-color:#232946; color:#E0E7FF;");
...
void LibrarianDashboard::onAddTable() {
    ...
    dlg.setStyleSheet("background-color:#232946;");
    auto* fl = new QFormLayout(&dlg);
...
void LibrarianDashboard::onViewUserDetails() {
    ...
    QString info =
        ...
        .arg(ur->balance,'f',2).arg(ur->total_fines_paid,'f',2)
