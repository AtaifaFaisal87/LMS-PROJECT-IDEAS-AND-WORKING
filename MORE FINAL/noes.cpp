...
//date format:--/--/---- // time format: --:-- (24-hour)
time_t reservationToTimeT(string date, string timeStr)
{
    struct tm t = {};  // zero out all fields first
    //on zeroth index day is stored, on 3rd index month is stored and on 6th index year is stored
    t.tm_mday = stoi(date.substr(0, 2)); //from zero index going forward pick two characters 
    t.tm_mon = stoi(date.substr(3, 2)) - 1;//from third index going forward pick two characters and subtract 1 because tm_mon is 0-based
    t.tm_year = stoi(date.substr(6, 4)) - 1900;//from sixth index going forward pick four characters and subtract 1900 because tm_year is years since 1900
    t.tm_hour = stoi(timeStr.substr(0, 2));//from zero index going forward pick two characters for hours
    t.tm_min = stoi(timeStr.substr(3, 2));//from third index going forward pick two characters for minutes4
    t.tm_sec = 0;// seconds set to 0 for reservation times
    t.tm_isdst = -1; //automatically determine daylight saving time
    return mktime(&t);//built in function that converts tm struct to time_t (system time format)
}
...
public:
    ...
    // RESTORE CONSTRUCTOR (for loading from file â€” does not reset times)
...
//
void loadUsersFromFile(vector<User *> &users) //file saye lekr user vector
{
    ifstream file("users.dat", ios::binary);
    if (!file)  //when file does not exist 
        return;
    ...
    {
        User *u = nullptr; // temporary pointer to hold the created user
        string type = ur.type;

        if (type == "STUDENT")//if user type is student then create student object
...
{
    ofstream file("users.dat", ios::binary); // no ios::app â€” overwrites entire file
...
public:
    ...
    // The following search functions return a vector of pointers because there can be multiple matches (e.g. multiple books by the same author)
    ...
    void ViewBookCatalog() const
    {
        ...
        //bk is a container from which each book pointer is extracted and printed using the overloaded << operator of the Book class
        for (Book *b : BK)
            cout << *b;
    }
...
void rewriteReservationsFile(vector<Reservation> &reservations)
{
    ...
    {
        // Write ALL reservations â€” active, expired, and cancelled
        ReservationRecord rr = {};
...
private:
    ...
    vector<User *> users;             // was: User* users[100] (polymorphic Ã¢â‚¬â€ kept as pointer)
