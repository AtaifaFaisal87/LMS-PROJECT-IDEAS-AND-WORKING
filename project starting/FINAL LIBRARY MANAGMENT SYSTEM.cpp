...
public:
    ...
    void displaylibrarian_details() // FRIEND OSTREAM
    {
...
private:
    ...
    vector<User *> users;             // was: User* users[100] (polymorphic — kept as pointer)
...
public:
    ...
    bool borrowBook(User *u, string title, string issue, string due)
    {
        ...
        if (u->borrowbook(book->get_bookid()))
        {
            ...
            transactions.emplace_back(u->get_userid(), book->get_bookid(), issue, due);
    ...
    bool returnBook(User *u, string title, int daysLate = 0)
    {
        ...
        if (u->returnbook(book->get_bookid()))
        {
            ...
            if (daysLate > 0)
            {
                fines.emplace_back(u->get_userid(), book->get_bookid(), daysLate, u); // emplace_back() is a function of vector that:
                cout << "FINE GENERATED!" << endl;                                    // Creates an object directly inside the vector (at the end) using given values
            }
            ...
            for (int i = 0; i < transactions.size(); i++)
            {
                if (transactions[i].getbookid() == book->get_bookid())
                {
                    if (transactions[i].getuserid() == u->get_userid())
                    {
                        ...
                        {
                            transactions[i].markreturned("RETURNED");
                            break;
                        }
                    }
                }
    ...
    void reserveBook(int uid, int bkid, string date, string time)
    {
        ...
        reservations.emplace_back(uid, bkid, 0, date, time);
    ...
    {
        reservations.emplace_back(uid, 0, tblid, date, time);
